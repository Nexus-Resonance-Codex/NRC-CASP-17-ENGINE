...
def test_contacts():
    ...
    contacts = [(0, 19, 6.0, 100.0), (4, 15, 6.0, 100.0)]
    ...
    print(
        f"Without contacts: dist(0, 19) = {dist_0_19_no:.2f} A, dist(4, 15) = {dist_4_15_no:.2f} A"
    )
    print(
        f"With contacts:    dist(0, 19) = {dist_0_19_yes:.2f} A, dist(4, 15) = {dist_4_15_yes:.2f} A"
    )

    # Verify that the contacts are significantly closer in the constrained run
    assert abs(dist_0_19_yes - 6.0) < 1.0, (
        "Residues 0 and 19 should satisfy target restraint of 6.0"
    )
    assert abs(dist_4_15_yes - 6.0) < 1.0, (
        "Residues 4 and 15 should satisfy target restraint of 6.0"
    )
    assert dist_0_19_yes < 8.0, (
        "Residues 0 and 19 should satisfy target restraint (< 8.0 A)"
    )
    assert dist_4_15_yes < 8.0, (
        "Residues 4 and 15 should satisfy target restraint (< 8.0 A)"
    )
