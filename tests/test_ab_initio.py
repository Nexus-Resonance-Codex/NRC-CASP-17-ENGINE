...
def test_ab_initio_folding():
    ...
    analysis = BiophysicsSuite.analyze_sequence(sequence, coords1, final_frame1["confidence"])
    assert "pI" in analysis
    
    audit = TTT7Auditor.audit_coordinates(coords1)
    assert "is_stable" in audit
    print(f"Ab initio folding test completed. Determinism verified. TTT-7 Status: {audit['status']}")
