...
class ScientificDeposition:
    ...
    def create_zenodo_draft(self, sequence: str, pdb_content: str, metadata: dict):
        ...
        if not self.zenodo_token:
            return self._generate_manual_manifest(
                sequence, pdb_content, metadata, "Zenodo"
            )

        # Logic for Zenodo API would go here (requires valid token)
        return {
            "status": "STUB",
            "message": "Zenodo API requires active token. Local manifest generated.",
        }

    def _generate_manual_manifest(
        self, sequence: str, pdb_content: str, metadata: dict, target: str
    ):
        ...
        manifest = {
            ...
            "metadata": {
                ...
                "creators": [
                    {"name": "Nexus Resonance Codex (NRC) (NRC) (NRC) Protocol"}
                ],
