...
def main():
    ...
    parser.add_argument(
        "--sequence", type=str, required=True, help="Amino acid sequence to fold"
    )
    parser.add_argument(
        "--pdb_id",
        type=str,
        default=None,
        help="Optional PDB ID of native structure for RMSD validation",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="folded_structure.pdb",
        help="Output path for the folded PDB file",
    )
    parser.add_argument(
        "--steps", type=int, default=40, help="Number of refinement steps"
    )
    parser.add_argument(
        "--optimizer",
        type=str,
        choices=["lbfgs", "annealing"],
        default="lbfgs",
        help="Optimizer routine to use",
    )
    ...
    fold_gen = engine.fold_sequence(args.sequence, k_guide=0.0, steps=args.steps)
