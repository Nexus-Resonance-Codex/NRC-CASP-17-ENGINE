...
if os.environ.get("SPACE_ID"):
    print(
        f"φ^∞ NRC: Running on Hugging Face ({os.environ.get('SPACE_ID')}). Redirecting CWD to /tmp."
    )
    os.chdir("/tmp")
...
from nrc_casp17_engine import NRCEngine, BiophysicsSuite, ReportingSuite
...
def split_pdb_by_chains(pdb_path):
    ...
    for cid in sorted(chains.keys()):
        temp_f = tempfile.NamedTemporaryFile(
            suffix=f"_{cid}.pdb", delete=False, mode="w"
        )
...
def extract_subunits_from_pdb(pdb_path):
    ...
    for cid in sorted(chains_data.keys()):
        ...
        subunits.append({"id": cid, "sequence": sequence})
    return subunits
    ...
    for i in range(0, len(grid_points), batch_size):
        ...
        for idx, is_pot in enumerate(potential_pocket):
            if is_pot:
                ...
                directions_hit = sum(
                    [has_pos_x, has_neg_x, has_pos_y, has_neg_y, has_pos_z, has_neg_z]
                )
                if directions_hit >= 4:
...
def get_viewer_html(refined_pdb, original_pdb=None, pockets=None):
    ...
    orig_safe = (
        original_pdb.replace("`", "\\`").replace("$", "\\$").replace("\n", "\\n")
        if original_pdb
        else ""
    )
...
def run_nrc_refinement_pipeline(
    pdb_file, seq_input, k_guide, steps, use_annealing, viewer_type
):
    logs = [
        f"[{datetime.now().strftime('%H:%M:%S')}] INITIALIZING RESONANCE PDB REFINER..."
    ]
    yield ["\n".join(logs)] + [None] * 16
    ...
    try:
        ...
        if pdb_file is not None:
            logs.append(
                f"[{datetime.now().strftime('%H:%M:%S')}] PARSING UPLOADED PDB FILE..."
            )
            yield ["\n".join(logs)] + [None] * 16
            ...
            logs.append(
                f"[OK] DETECTED {len(subunits)} CHAIN(S) WITH SEQUENCE LENGTH: {len(seq)}"
            )
        else:
            # Fallback to Sequence Input (Ab Initio Mode)
            seq = seq_input.strip().upper().replace("\n", "").replace(" ", "")
            if not seq:
                yield ["[ERROR] NO PDB FILE UPLOADED OR SEQUENCE PROVIDED."] + [
                    None
                ] * 16
                return
            ...
            logs.append(
                f"[{datetime.now().strftime('%H:%M:%S')}] RUNNING AB INITIO FOLDING ON SEQUENCE LENGTH: {len(seq)}..."
            )
        ...
        for frame in engine.fold_complex(
            ...
            if step == 1:
                logs.append(
                    f"[{datetime.now().strftime('%H:%M:%S')}] STAGE 1: GEOMETRIC COMPACTION & TTT-7 REGULARIZATION"
                )
            elif step == max(1, steps // 2):
                logs.append(
                    f"[{datetime.now().strftime('%H:%M:%S')}] STAGE 2: BIOPHYSICAL RELAXATION & CLASH RESOLUTION"
                )

            yield [
                "\n".join(logs + [f"Minimizing structure... Step {step}/{steps}"])
            ] + [None] * 16
            if frame["final"]:
                final_frame = frame
        ...
        refined_pdb_text = ReportingSuite.generate_pdb(
            seq,
            coords_opt,
            confidence,
            all_atom=True,
        ...
        # 4. Pocket Detection
        logs.append(
            f"[{datetime.now().strftime('%H:%M:%S')}] RUNNING BINDING POCKET DETECTION..."
        )
        yield ["\n".join(logs)] + [None] * 16
        ...
        if original_pdb_text:
            orig_coords, orig_res_idx, orig_atom_types, orig_chain_ids, _ = (
                parse_pdb_all(pdb_file.name)
            )
            ...
            for i in range(n_orig):
                for j in range(i + 1, n_orig):
                    if (
                        orig_res_idx[i] == orig_res_idx[j]
                        and orig_chain_ids[i] == orig_chain_ids[j]
                    ):
                        continue
                    if (
                        orig_chain_ids[i] == orig_chain_ids[j]
                        and abs(orig_res_idx[i] - orig_res_idx[j]) == 1
                    ):
                        if (
                            orig_atom_types[i] == "C" and orig_atom_types[j] == "N"
                        ) or (orig_atom_types[i] == "N" and orig_atom_types[j] == "C"):
                            continue
            ...
            original_rg = np.sqrt(
                np.mean(np.sum((orig_coords - mean_orig) ** 2, axis=1))
            )
        ...
        for i in range(n_refined):
            for j in range(i + 1, n_refined):
                ...
                if (
                    ...
                    and abs(res_indices[i] - res_indices[j]) == 1
                ):
                    if (atom_types[i] == "C" and atom_types[j] == "N") or (
                        atom_types[i] == "N" and atom_types[j] == "C"
                    ):
                        continue
        ...
        if original_pdb_text:
            ...
            if m_len > 0:
                ...
                rmsd_val = float(
                    np.sqrt(np.mean(np.sum((c1_rotated - c2_centered) ** 2, axis=1)))
                )

        # 6. Build Summary Table
        summary_data = [
            [
                "RMSD to Input Structure",
                "-",
                f"{rmsd_val:.4f} Å" if original_pdb_text else "N/A",
            ],
            [
                "Steric Clashes (< 1.30 Å)",
                str(original_clashes) if original_pdb_text else "-",
                str(refined_clashes),
            ],
            [
                "Min Inter-Atomic Distance",
                f"{original_min_dist:.4f} Å" if original_pdb_text else "-",
                f"{refined_min_dist:.4f} Å",
            ],
            [
                "Radius of Gyration (Rg)",
                f"{original_rg:.4f} Å" if original_pdb_text else "-",
                f"{refined_rg:.4f} Å",
            ],
            [
                "TTT-7 Stability Digital Root",
                "-",
                str(int(round(np.sum(np.abs(coords_opt)) * 1000.0) - 1) % 9 + 1),
            ],
        ]
        summary_df = pd.DataFrame(
            summary_data,
            columns=["Biophysical Metric", "Before Refinement", "After Refinement"],
        )
        ...
        with open(pockets_path, "w") as f:
            ...
            for p in pockets:
                ...
                f.write(
                    f"{p['id']},{p['center'][0]:.3f},{p['center'][1]:.3f},{p['center'][2]:.3f},{p['volume']:.1f},{res_str}\n"
                )
        ...
        with open(profile_path, "w") as f:
            f.write(
                "residue_index,residue_name,hydropathy_score,charge,dssp_assignment\n"
            )
            for idx, rname in enumerate(res_names):
                if atom_types[idx] == "CA":
                    ...
                    dssp = (
                        analysis["dssp"][res_indices[idx] - 1]
                        if res_indices[idx] - 1 < len(analysis["dssp"])
                        else "-"
                    )
                    f.write(
                        f"{res_indices[idx]},{rname},{hydropathy},{charge},{dssp}\n"
                    )
        ...
        with open(report_path, "w") as f:
            ...
            f.write("==================================================\n")
            f.write(
                f"Refinement Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
            )
            ...
            for p in pockets:
                f.write(
                    f"Pocket {p['id']} - Center: {p['center']} | Volume: {p['volume']:.1f} A^3\n"
                )
        ...
        zip_out = os.path.join(
            tempfile.gettempdir(),
            f"nrc_refinement_package_{int(datetime.now().timestamp())}.zip",
        )
        ...
        phi_angles, psi_angles = BiophysicsSuite.calculate_phi_psi(
            coords_opt[np.array(atom_types) == "CA"]
        )
        rama_fig = px.scatter(
            x=phi_angles,
            y=psi_angles,
            labels={"x": "Phi (deg)", "y": "Psi (deg)"},
            title="Pseudo-Ramachandran Plot",
        ...
        h_fig = px.line(
            x=indices,
            y=analysis["hydropathy"],
            title="Kyte-Doolittle Hydropathy Profile",
            template="plotly_dark",
        )
        # Charge profile plot
        ch_fig = px.bar(
            x=indices,
            y=analysis["charge"],
            title="Residue Charge Distribution",
            template="plotly_dark",
        )
        ...
        for c in ca_coords:
            ...
            ttt_roots.append(dr)
        conf_fig = px.line(
            x=np.arange(len(ttt_roots)),
            y=ttt_roots,
            title="TTT-7 Digital Root Profile",
            template="plotly_dark",
        )
        ...
        logs.append("[OK] REFINEMENT SEQUENCE COMPLETE. 100% MATH PARITY ACHIEVED.")
        yield [
            "\n".join(logs),
    ...
    with gr.Row():
        with gr.Column(scale=1):
            with gr.Column(elem_classes="premium-card"):
                ...
                pdb_file = gr.File(
                    label="Upload Protein PDB File (.pdb)", file_types=[".pdb"]
                )
                seq_input = gr.Textbox(
                    label="Or Enter Sequence (Ab Initio Folding Mode)",
                    lines=3,
                    placeholder="MTVKV...",
                )
            ...
            with gr.Column(elem_classes="premium-card"):
                ...
                k_guide = gr.Slider(
                    ...
                    step=0.1,
                    value=0.5,
                    label="Refinement Strength (k_guide)",
                    info="0.0 = pure unconstrained relaxation, 0.5+ = guided backbone constraint",
                )
                steps = gr.Slider(
                    minimum=10,
                    maximum=100,
                ...
                use_annealing = gr.Checkbox(
                    label="Simulated Annealing Relaxation",
                    value=False,
                    info="Run Monte Carlo annealing cycles to traverse local minima",
                )
                viewer_type = gr.Radio(["3Dmol"], label="Viewer Engine", value="3Dmol")

            fold_btn = gr.Button(
                "Refine Structure & Detect Pockets",
                variant="primary",
            ...
            with gr.Tabs(elem_classes="tabs") as tabs_manifold:
                ...
                with gr.Tab("Biophysical Analytics", id="results_tab"):
                    ...
                    with gr.Row():
                        ...
                        m_plot = gr.Plot(
                            visible=False
                        )  # Keep hidden for output compatibility

                with gr.Tab("Process Console Logs", id="log_tab"):
                    status_log = gr.Textbox(
                        label="Forcefield Minimization Log",
                        lines=10,
...
- **Model 5**: Direct template projection (no relaxation)
                    ...
                    with gr.Row():
                        batch_pdb = gr.File(
                            label="Upload Guide PDB for Batch Folding",
                            file_types=[".pdb"],
                        )
                        batch_target_id = gr.Textbox(
                            label="Target ID (e.g. T1406)", placeholder="H2381"
                        )
                    batch_btn = gr.Button(
                        "Generate 5 Models & Download ZIP", variant="primary"
                    )
                    batch_out = gr.File(label="Download 5-Model ZIP Package")
                    batch_log = gr.Textbox(
                        label="Batch Run Log", lines=5, elem_classes="log-console"
                    )
                ...
                with gr.Tab("Research Package Export"):
                    with gr.Row():
                        ...
                        pdb_code = gr.Code(
                            label="Refined PDB Source Preview", language="markdown"
                        )
    ...
    def run_batch_5_models(pdb_file, target_id):
        ...
        for m_idx in range(5):
            logs.append(
                f"[BATCH] Running Model {m_idx + 1}/5 (k_guide={k_guides[m_idx]})..."
            )
            try:
                final_frame = None
                for frame in engine.fold_complex(
                ...
                if final_frame:
                    ...
                    pdb_path = os.path.join(
                        temp_dir, f"{target_id}_{model_names[m_idx]}.pdb"
                    )
        ...
        zip_path = os.path.join(
            tempfile.gettempdir(),
            f"{target_id}_5models_{int(datetime.now().timestamp())}.zip",
        )
